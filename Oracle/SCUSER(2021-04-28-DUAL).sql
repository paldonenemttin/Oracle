...
-- Oracle에서는 FROM 절이 있는 SELECT를 허용하지 않는다
...
-- 소수 첫째자리에서 반올ㄹ미하여 정수 표현
SELECT ROUND(333.33) AS 삼삼,
       ROUND(333.44) AS 사사,
       ROUND(333.55) AS 오오 FROM DUAL;

-- 소수 둘째 자리에서 반오림하여 첫째 자리까지 표현       
SELECT ROUND(333.33 ,1) AS 삼삼,
       ROUND(333.44,1) AS 사사,
       ROUND(333.55,1) AS 오오 FROM DUAL;
       
-- 일반적인 코딩과 달리 정수 부분도 반올림 연산을 수행 할 수 있다
SELECT ROUND(555,-1) AS 삼삼,
       ROUND(555, -2) AS 사사,
       ROUND(555, -3) AS 오오 FROM DUAL;
       
-- 단순 문자열 출력
SELECT '2021-04-28' FROM DUAL;

-- 문자열 연결할때 +를 사용하지 않고 || 를 사용
...
-- DBMS의 자동 형변환
-- + 연산을 수행하면 문자열형 숫자데이터를 실제 숫자형으로 변환하여 연산을 수행
-- 자동 형변환은 예상하지 못하는 연산결과를 얻을 수 있다
SELECT '30' + '40' FROM DUAL;

-- 날짜와 시간데이터에 ROUND를 입히기
-- 보이는 것과 달리 실제 데이터는
-- 2021-04-28:00:00:00 형식으로 ROUND가 사용된다
SELECT 
ROUND(TO_DATE('2021-04-2811:27','yyyy-mm-ddhh24:mi:ss'))
FROM DUAL;
...
SELECT CASE WHEN MOD(11,2) = 1 THEN '홀수' END CASE
FROM DUAL;