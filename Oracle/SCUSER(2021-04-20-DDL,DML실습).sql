-- 여기는 SCUSER로 접속한 화면
-- SCUSER는 SBA권한을 가지고 있으며
-- 데이터를 저장하기 위한 TABLE을 만들고
-- 데이터를 추가ㅡ 조회, 수정, 삭제 실습하기
...
-- 표(TABLE) 형식으로 저장된 것으로인식하는 표준형식이다
-- ORACLE의 데이터 저장구조 : TABLESPACE > TABLE > 데이터들
-- TABLE은 JAVA 코딩에서 VO클래스를 만드는 원리와 비슷하다
--     데이터를 취급하는 최소한의 귝겨, 형식을 지정하여생성한다

-- DDL 명령중에  TABLE생성학
--  TABLE은 데이터를 저장하기 위한 표는 작성하는 것으로
--  각 데이터의 이름을 저장하고, 각 데이터의 형식(TYPE)을 지정하여
-- 표를 작성한다

-- 데이터Type
-- 문자열 데이터
-- CHAR : 저장되는 데이터의 길이 (규칙으로) 정해진경우.
--        데이터의 길이가 모두 같은경우
-- VARCHAR : 저장되는 데이터의 최대크기만 지정하고
--           최대 크기보다 작은 데이터를 저장 할 수 있다
--           최대 크기보다 작은 데이터를 저장하면
--           자동으로 공간을 축소하여 데이터를 저장한다
--           "가변길이 문자열"
-- nVARCHAR : 한글과 같은 UNICORD 문자열을 저장할때 
--            영문 알파벡과 저장하는 방식이 달라서 문제를 일으킬 수 있다
--            한글과 같은 UNICORD 데이터를 저장하는 칼럼은
--            nVARCHAR 키워드를 사용한다
-- 숫자데이터
-- 정수, 실수 등을 구분하여 저장하기도 하지만
-- 오라클에서는 NUMBER 키워드로 일광 지정하여도 상관없다
-- 저장되는 자릿수에 따라 자동으로 크키가 조절된다
...
-- DML(Data Maniqulation Lang. Data Management Lang. 데이터 조작어)
